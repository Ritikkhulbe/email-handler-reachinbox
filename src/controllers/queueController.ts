...
async function init(body: EmailBody) {
...
export const sendMailViaQueue = async (req: { params: RequestParams; body: RequestBody }, res: any) => {
  try {
...
export const sendMultipleEmails = async (req: { params: { id: string }; body: { from: string; to: string | string[] } }, res: any) => {
  try {
    const { id } = req.params;
    const { from, to } = req.body;
...
const sendEmailToQueue = async ({ from, to, id }: { from: string; to: string; id: string }) => {
  try {
...
export const sendMail = async (data: ParsedEmailData, token: string) => {
  try {
    if (!token) {
    ...
    if(!response.choices[0].message.content) throw new Error("OpenAI response content not found")
...
export const parseAndSendMail = async (data1: EmailBody, token: string) => {
  try {
    const { from, to } = data1;
    if (!token) {
      throw new Error("Token not found, please login again to get token");
    ...
    if(!prediction) throw new Error("No content from openai");
